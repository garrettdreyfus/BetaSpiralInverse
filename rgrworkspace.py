...
#profiles = rgr.extractArgoProfiles(os.path.abspath("data/rgrargonc"))
# profiles = rgr.extractBodcProfiles(os.path.abspath("data/rgrmorectd/"))
# print("BODC: ",len(profiles))
# profiles = rgr.extractWoceProfiles(os.path.abspath("data/rgrnc/"))
...
# #graph.plotProfiles(rgr,profiles,"",specialprofile=profilechoice)
...
# with open('data/annotatedrgrargowoce.pickle', 'wb') as outfile:
...
# preinterpsurfaces = nstools.addDataToSurfaces(rgr,profiles,preinterpsurfaces)
# #nstools.addGammaN(surfaces)


# with open('data/rgrsurfaceswdata.pickle', 'wb') as infile:
#     pickle.dump([preinterpsurfaces,profiles],infile)
# with open('data/rgrsurfaceswdata.pickle', 'rb') as outfile:
#     preinterpsurfaces,profiles = pickle.load(outfile)
# # print("hi")
...
#     interptools.interpolateSurfaces(rgr,preinterpsurfaces,\
#     interpmethod="gam",smart=False,coord="latlon",splines=30)
# #nstools.neutralityError(surfaces)
# #graph.graphSurfaces(rgr,surfaces,"nserror",stds=2,show=False,savepath="../arcticcirc-pics/surfaces/normneutralerror/")
# #graph.nsHist(surfaces)


# with open('data/interpedrgr.pickle', 'wb') as outfile:
#     pickle.dump([surfaces,neighbors,distances], outfile)
# with open('data/interpedrgr.pickle', 'rb') as outfile:
#     [surfaces,neighbors,distances] = pickle.load(outfile)

# surfaces = nstools.addParametersToSurfaces(rgr,surfaces,neighbors,distances)
...
# with open('data/invertedrgr.pickle', 'wb') as outfile:
#     pickle.dump([inv,neighbors,distances], outfile)
with open('data/invertedrgr.pickle', 'rb') as outfile:
    [inv,neighbors,distances] = pickle.load(outfile)
print(nstools.regionCurl(inv,3600,-39,-36,-38,-27))
graph.graphVectorField(rgr,inv,"uabs","vabs","pres",# \
...
# graph.fourpanelVectorField(rgr,inv,"uabs","vabs",backgroundfield="s",\
#                            select=[1054,1779,3201,4466],transform=False,scale=0.1)
# graph.graphSurfaces(rgr,inv,"pres",stds=2,show=True,select=range(3200,5000),contour=True)
# kvs = []
# for l in inv.keys():
...
# #graph.graphSurfaces(rgr,inv,"bathvar",stds=1,show=True,select=range(200,5000))
...
with open('data/interpedrgr.pickle', 'rb') as outfile:
    [surfaces,neighbors,distances] = pickle.load(outfile)
#graph.graphSurfaces(rgr,surfaces,"psi",stds=1,show=True,select=range(3000,5000))
...
#graph.graphSurfaces(rgr,inv,"gamma",stds=2,savepath="refpics/surfaces/gammaderivedsurface/",show=False)
# graph.graphVectorField(rgr,out,"uabs","vabs","pres",\
#         transform=False)
# graph.graphVectorField(rgr,inv,"uabs","vabs","pres",# \
        # metadata=out["metadata"],\
        # transform=False,show=False,
        # savepath="refpics/vectorfields/rgrwoceargo/",scale=0.1)
