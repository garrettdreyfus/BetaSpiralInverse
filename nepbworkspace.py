...
#print(profilechoice)
#surfaces = nstools.runPeerSearch(profiles,range(100,4000,200),profilechoice,False,10**10)
...
#surfaces,neighbors,distances = interptools.interpolateSurfaces(preinterpsurfaces,\
        #regions.nepb,interpmethod="gam",smart=True,coord="latlon")
...
#surfaces = nstools.addParametersToSurfaces(surfaces,\
        #neighbors,distances,regions.nepb,[])
...
#for k in inv.keys():
    ...
    #print(k,": kvo avg: ",np.nanmean(inv[k]["data"]["kvo"]))
    #print(k,": kvo std: ",np.nanstd(inv[k]["data"]["kvo"]))
    #print(k,": kvh avg: ",np.nanmean(inv[k]["data"]["kh"]))
    #print(k,": kvh std: ",np.nanstd(inv[k]["data"]["kh"]))
    #print(k,": psisol avg: ",np.nanmean(inv[k]["data"]["psisol"]))
#graph.graphSurfaces(inv,"kh",region="nepb")
#graph.graphVectorField(inv,"uabs","vabs","e",\
        #metadata=out["metadata"],region="nepb",\
        #transform=False)

matlabout = {"nspres":[],"pres":[],"u":[],"v":[],"e":[],"lat":[],"lon":[]}

for k in inv.keys():
    matlabout["nspres"].append(k)
    matlabout["u"].append(inv[k]["data"]["uabs"])
    matlabout["pres"].append(inv[k]["data"]["pres"])
    matlabout["v"].append(inv[k]["data"]["vabs"])
    matlabout["lat"].append(inv[k]["lats"])
    matlabout["lon"].append(inv[k]["lons"])
    if "e" in inv[k]["data"].keys() and k in range(1100,3600) :
        matlabout["e"].append(inv[k]["data"]["e"])
    else:
        matlabout["e"].append(inv[k]["lons"]*0)
...
#graph.graphVectorField(inv,"uabs","vabs","z",\
        ...
        #savepath="refpics/vectorfields/nepbpeer/")
...
#graph.graphVectorField(inv,"uabs","vabs","kh",\
        #metadata=out["metadata"],region="nepb",\
